...
app = FastAPI(title="modbus采集查询服务 V3")
...
@app.get("/api/blacklist/remove")
async def remove_blacklist(slave_id: int):
    if redis_client is None:
        raise HTTPException(status_code=500, detail="redis未初始化，熔断功能不可用")
    redis_client.delete(f"blacklist:{slave_id}")
    return {"msg": f"slave {slave_id} 移出熔断黑名单，可重新采集"}

# ===================== Redis熔断黑名单工具函数 V3新增 =====================
def is_slave_in_blacklist(slave_id: int) -> bool:
    """判断从站是否处于熔断黑名单，存在则跳过采集"""
    if redis_client is None:
        return False
    return redis_client.exists(f"blacklist:{slave_id}") > 0

def add_slave_blacklist(slave_id: int, expire_sec: int = 300):
    """故障从站加入黑名单，默认冷却5分钟"""
    if redis_client is None:
        return
    redis_client.setex(f"blacklist:{slave_id}", expire_sec, "1")
    logger.warning(f"【熔断】slave {slave_id} 加入黑名单，冷却{expire_sec}秒，暂停采集")
...
def slave_online_monitor(slave_id:int, success:bool) -> int:
    ...
    if success:
        ...
        if info["offline"] ==0 and info["fail_cnt"] >= slave_offline_cnt:
            ...
            add_slave_blacklist(slave_id, expire_sec=300)
    return info["offline"]
...
def config_validate(cfg: dict):
    required_keys = [
        ...
        "mem_warn_threshold_mb", "mysql", "modbus", "redis", "slave_dict"
    ]
    missing = []
...
# ===================== 程序启动自检（新增Redis自检 V3） =====================
def startup_self_check(mysql_cfg:dict, modbus_cfg:dict, redis_cfg:dict):
    global redis_client
...
# Redis自检初始化
    try:
        ...
        logger.info("🟢 A6 Redis连接正常，熔断黑名单功能启用")
    except Exception as e:
        logger.error(f"🟢 A6 Redis连接失败 ({e})，熔断黑名单功能关闭")
        redis_client = None
...
def _inner_collect_loop(slave_id:int, interval:int, modbus_cfg):
    ...
    while not SHUTDOWN_FLAG:
        # V3 熔断判断：黑名单内直接跳过采集
        if is_slave_in_blacklist(slave_id):
            time.sleep(1)
            continue
        start_ts = time.time()
...
# ===================== 主入口 =====================
def main():
    global RUNTIME_CONFIG, DATA_QUEUE
    ...
    startup_self_check(mysql_cfg, modbus_cfg, redis_cfg)

    DATA_QUEUE = Queue(maxsize=RUNTIME_CONFIG["data_queue_maxsize"])
    mysql_pool = MysqlPool(mysql_cfg, max_idle=4)
    # 启动补偿回放离线缓存
    load_and_replay_to_sql(mysql_pool)
    slave_dict = RUNTIME_CONFIG["slave_dict"]

    # 后台线程启动
    threading.Thread(target=mem_monitor_thread, daemon=True).start()
    ...
    for _ in range(consumer_num):
        t = threading.Thread(target=consumer_thread_wrapped, args=(mysql_pool,), daemon=True)
        t.start()

    # 每个从站独立采集线程
    for slave_id, interval in slave_dict.items():
        t = threading.Thread(target=slave_collect_thread_wrapped, args=(slave_id, interval, modbus_cfg), daemon=True)
        t.start()

    # 主线程阻塞等待停机信号
    while not SHUTDOWN_FLAG:
        time.sleep(1)
    logger.info("等待剩余数据消费完成，准备退出")
    wait_max = RUNTIME_CONFIG["shutdown_max_wait"]
    wait_start = time.time()
    while (not DATA_QUEUE.empty()) and (time.time()-wait_start < wait_max):
        time.sleep(0.3)
    mysql_pool.close_all()
    logger.info("网关V3服务退出完成")
